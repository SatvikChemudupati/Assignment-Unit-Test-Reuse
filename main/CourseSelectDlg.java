...
public class CourseSelectDlg extends JDialog {
	...
	int nCourseLevel = 0;
	boolean menuLogout = false;
	JComboBox<Course> courseNameCombox = new JComboBox<>();
	...
	private void jbInit() {
		...
		courseNameCombox.setBounds(new Rectangle(155, 41, 203, 22));
		...
		this.getContentPane().add(courseNameCombox, null);
	...
	public Course showDlg(ClassCourseList courseList) {
		...
		{
			courseNameCombox.addItem(theCourse);
		}
		setVisible(true);
		return selectedCourse;
	}

	void okButton_actionPerformed(ActionEvent ignoredE) {
		selectedCourse = (Course) courseNameCombox.getSelectedItem();
